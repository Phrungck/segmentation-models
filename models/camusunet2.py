...
class CamusUnet2(nn.Module):
    def __init__(self, n_channels, n_classes, ch_sizes=[48, 96, 192, 384, 768]):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # encoder path
        enc_1 = self.conv1(x)  # B, 48, 256, 256
        enc_2 = self.down[0](enc_1)  # B, 96, 128, 128
        enc_3 = self.down[1](enc_2)  # B, 192, 64, 64
        enc_4 = self.down[2](enc_3)  # B, 384, 32, 32

        bridge = self.down[3](enc_4)  # B, 768, 16, 16

        # decoder path
        out = self.up[0](bridge, enc_4)  # B, 384, 32, 32
        out = self.up[1](out, enc_3)  # B, 192, 64, 64
        out = self.up[2](out, enc_2)  # B, 96, 128, 128
        out = self.up[3](out, enc_1)  # B, 48, 256, 256
